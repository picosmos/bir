...
// cspell:words VCALENDAR PRODID CALDESC CALSCALE VEVENT DTSTAMP DTSTART DTEND yyyyMMddTHHmmssZ CALNAME VALARM
...
public class IcsService : IIcsService
{
    ...
    private static string GenerateUid(CalendarEvent calendarEvent)
    {
        // Generate a unique ID based on the event details
        var source = $"{calendarEvent.Date:yyyyMMddHHmmss}-{calendarEvent.Title}";
        var hash = source.GetHashCode();
        return $"{hash:X}@bir.calendar";
    }
